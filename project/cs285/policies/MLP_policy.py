...
class MLPPolicy(BasePolicy):
    ...
    def get_action(self, obs):
        ...
        #print(obs)
        return self.sess.run(self.sample_ac, feed_dict={self.observations_pl:np.array([obs])})
...
class MLPPolicyPG(MLPPolicy):
    ...
    def define_train_op(self):
        ...
        self.loss = tf.reduce_sum(-1*tf.multiply(self.logprob_n,self.adv_n))
    ...
    def update(self, observations, acs_na, adv_n=None, acs_labels_na=None, qvals=None):
        ...
        if self.nn_baseline:
            ...
            _ = self.sess.run([self.baseline_update_op], feed_dict={self.targets_n: targets_n, self.observations_pl: observations}) # TODO
        return loss
